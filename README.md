...
项目采用 Flask 应用工厂与 Blueprint 分模块路由，业务逻辑集中在 `utils/`，前端使用原生 HTML、CSS 和 JavaScript，图表使用 Plotly.js。整个应用不依赖数据库，适合本地运行、课程实验和小规模数据分析。
...
以上八个模块已接入页面路由和应用导航。仓库中另有 `dimension_mining` 维度挖掘代码，当前尚未注册到应用工厂和页面路由，因此暂不作为可用功能对外承诺。
...
当前仓库尚未声明开源许可证。公开发布前，请根据项目实际授权方式补充 LICENSE 文件。
