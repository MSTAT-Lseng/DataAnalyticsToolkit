...
def _register_routes(app: Flask) -> None:
    ...
    # ---- 维度挖掘预览 API ----
    @app.route("/api/dimension-mining/preview", methods=["POST"])
    def api_dimension_mining_preview():
        ...
        try:
            ...
            app.logger.exception("Dimension mining preview API error")
            return jsonify({"success": False, "error": str(exc)}), 500

    # ---- 维度挖掘分析 API ----
    @app.route("/api/dimension-mining/analyze", methods=["POST"])
        ...
        try:
            ...
            for i, dim in enumerate(dimensions):
                if not dim.get("name", "").strip():
                    return jsonify({
                        "success": False,
                        "error": f"第 {i + 1} 个维度缺少名称"
                    }), 400
                keywords = dim.get("keywords", [])
                if not keywords or not isinstance(keywords, list):
                    return jsonify({
                        "success": False,
                        "error": f"维度「{dim['name']}」至少需要一个关键词"
                    }), 400
                for j, kw in enumerate(keywords):
                    if not kw.get("pattern", "").strip():
                        return jsonify({
                            "success": False,
                            "error": f"维度「{dim['name']}」的第 {j + 1} 个关键词缺少匹配模式"
                        }), 400
            ...
            app.logger.exception("Dimension mining analyze API error")
            return jsonify({"success": False, "error": str(exc)}), 500

    # ---- 维度挖掘导出 API ----
    @app.route("/api/dimension-mining/export", methods=["POST"])
    def api_dimension_mining_export():
        ...
        try:
            ...
            return send_file(
                ...
                mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                as_attachment=True,
                download_name="维度挖掘结果.xlsx",
            )
        except Exception as exc:
            app.logger.exception("Dimension mining export API error")
            return jsonify({"success": False, "error": str(exc)}), 500
